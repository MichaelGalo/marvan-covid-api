...
class CARapidTestDemand(SQLModel, table=True):
    ...
    NAICS: str = Field(
        sa_column=Column(
            "North American Industry Classification System (NAICS)", String, quote=True
        )
    )
    RapidTestDemandUsage: str = Field(
        sa_column=Column(
            "COVID-19 rapid test kits demand and usage", String, quote=True
        )
    )
