...
class USDeathCounts(SQLModel, table=True):
    ...
    covid_deaths: Optional[int] = Field(
        sa_column=Column("covid_deaths", Integer, quote=True)
    )
    crude_covid_death_rate: Optional[int] = Field(
        sa_column=Column("crude_covid_death_rate", Integer, quote=True)
    )
    age_adjusted_covid_death_rate: Optional[int] = Field(
        sa_column=Column("age_adjusted_covid_death_rate", Integer, quote=True)
    )
    annualized_crude_covid_death_rate: Optional[int] = Field(
        sa_column=Column("annualized_crude_covid_death_rate", Integer, quote=True)
    )
    annualized_age_adjusted_covid_death_rate: Optional[int] = Field(
        sa_column=Column(
            "annualized_age_adjusted_covid_death_rate", Integer, quote=True
        )
    )
